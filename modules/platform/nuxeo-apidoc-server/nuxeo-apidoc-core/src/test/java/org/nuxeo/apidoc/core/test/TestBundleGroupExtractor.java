...
public class TestBundleGroupExtractor {
    ...
    public void test() {
        ...
        assertEquals("grp:org.nuxeo.common", roots.get(0).getId());
        assertEquals(List.of(), roots.get(0).getSubGroups());
        assertEquals(List.of(), roots.get(0).getParentIds());
        assertEquals("grp:org.nuxeo.ecm.core", roots.get(1).getId());
        assertEquals(List.of(), roots.get(1).getSubGroups());
        assertEquals(List.of(), roots.get(1).getParentIds());
        assertEquals("grp:org.nuxeo.ecm.platform", roots.get(2).getId());
        assertEquals(
                List.of("grp:org.nuxeo.apidoc", "grp:org.nuxeo.ecm.directory", "grp:org.nuxeo.ecm.platform.restapi",
                        "grp:org.nuxeo.ecm.relations"),
                roots.get(2).getSubGroups().stream().map(BundleGroup::getId).collect(Collectors.toList()));
        assertEquals(List.of(), roots.get(2).getParentIds());
        assertEquals("grp:org.nuxeo.runtime", roots.get(3).getId());
        assertEquals(List.of(), roots.get(3).getSubGroups());
        assertEquals(List.of(), roots.get(3).getParentIds());
